# Arsenal Transfer Alert：Arsenal 默认版详细运维手册

> 本页保留完整的运行、安全、成本与故障处理细节。面向新用户的简明说明见
> [中文 README](../README.zh-CN.md)；项目现已支持通过单个 TOML 配置目标俱乐部，
...
- DeepSeek 必须先返回 Arsenal 在当前事件中的固定参与角色：
  ...
  `none`。本地严格校验禁止 `club_participation=none` 的结果进入通知队列。
...
```text
.
├─ config/sources.toml              # Arsenal 默认俱乐部/消息源/Tier 配置
├─ config/sources.example.toml      # 其他俱乐部的最小配置模板
...
目标俱乐部和 Tier 均由 `config/sources.toml` 静态提供，DeepSeek 看不到修改 Tier 的入口。
配置只接受 Tier 0、1、2。其他球队可以复制 `config/sources.example.toml`，只填写
`[club]` 的 3 个必填项和每个 `[[sources]]` 的 5 个必填项；仓库默认文件仍是 Arsenal 版。
...
BARK_DEVICE_KEY=...
# 留空时使用 [club].notification_group；Arsenal 默认即 Arsenal Transfer Alert
BARK_GROUP=
BARK_LEVEL=active
DRY_RUN=true
...
模拟数据生成的示例见 [mock-bark-example.md](mock-bark-example.md)：
...
接口选择、计费快照和核对日期集中记录在
[API_RESEARCH.md](API_RESEARCH.md)。正式启用前必须重新打开官方页面核对。
